...
#ifndef YAMATO_ESP_YAMATO_H
#define YAMATO_ESP_YAMATO_H
...
class Yamato : public Climate, public PollingComponent {
...
public:
    ...
    Yamato() : PollingComponent(5 * 1000) {
        lastCRC = 0;
    }
    ...
    void update() override {
        ...
        ESP_LOGD("Yamato", "POLL: %s ", raw.c_str());
    }
...
public:
    ...
    void parseStatus() {
        ...
        ESP_LOGD("Yamato", "Readed message ALBA: %s ", raw.c_str());

        byte check = getChecksum(status, sizeof(status));

        if (check != status[CRC_OFFSET(status)]) {
            ESP_LOGW("Yamato", "Invalid checksum (%d vs %d)", check, status[CRC_OFFSET(status)]);
            return;
        }
        ...
        if(current_temperature < MIN_VALID_INTERNAL_TEMP || current_temperature > MAX_VALID_INTERNAL_TEMP 
            ...
            ESP_LOGW("Yamato", "Invalid temperatures");
            return;
        }
        ...
        if (GetPowerStatus() == false) {
			...
			if(GetVerticalSwingStatus() == VERTICAL_SWING_AUTO){
    ...
    void sendData(byte * message, byte size) {
        ...
        ESP_LOGD("Yamato", "Message sent: %s  - CRC: %X - CRC16: %X", raw.c_str(), crc, crc_16);
